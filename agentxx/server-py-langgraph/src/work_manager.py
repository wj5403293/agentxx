...
from typing import Annotated, Any, Awaitable, Callable, Literal, cast, override
...
class XXTodoItem(TypedDict):
    """A single todo item with content and status."""

    prefix: str
    """前置任务提示，可能包含对该任务祥光的主上下文的一些缩略等信息."""

    retryTip: str
    """如果任务项执行失败过，则重新执行前会总结失败的尝试，得出提示，优化下一次执行"""

    content: str
    """任务项内容."""

    status: Literal["pending", "in_progress", "checking", "success", "faild"]
    """任务项状态"""

    messageTree: list[list[BaseMessage]]
    """隔离消息树"""

    summary: str
    """任务项完成后的总结，这会插入到主模型的上下文中."""


class XXWorkPlanningState(AgentState[ResponseT]):
    ...
    todos: Annotated[NotRequired[list[XXTodoItem]], OmitFromInput]
    """List of todo items for tracking task progress."""


class XXWorkWriteTodosInput(BaseModel):
    """Input schema for the `write_todos` tool."""

    todos: list[XXTodoItem]
...
def write_todos(
    todos: list[XXTodoItem], tool_call_id: Annotated[str, InjectedToolCallId]
) -> Command[Any]:
...
def _write_todos(
    runtime: ToolRuntime[ContextT, XXWorkPlanningState[ResponseT]], todos: list[XXTodoItem]
) -> Command[Any]:
    """Create and manage a structured task list for your current work session."""
    return Command(
        update={
...
class XXWorkManagerMiddleware(AgentMiddleware[XXWorkPlanningState[ResponseT], ContextT, ResponseT]):

    state_schema = XXWorkPlanningState  # type: ignore[assignment]
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the `XXWorkManagerMiddleware` with optional custom prompts.
        ...
        self.tools = [
            StructuredTool.from_function(
                ...
                args_schema=XXWorkWriteTodosInput,
                infer_schema=False,
            )
        ]
    ...
    @override
    def after_model(
        self, state: XXWorkPlanningState[ResponseT], runtime: Runtime[ContextT]
    ) -> dict[str, Any] | None:
