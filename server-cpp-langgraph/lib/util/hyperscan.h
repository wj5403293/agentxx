...
public:
  // 初始化Hyperscan数据库
  XXRegex(const std::string &regstr, unsigned int flags = HS_FLAG_UTF8)
  ...
  inline static int xxregexMatchCallback(unsigned int id,
    ...
    // 提取并格式化当前匹配区间
    const unsigned int curr_start = static_cast<unsigned int>(from);
    const unsigned int curr_end = static_cast<unsigned int>(to);
    const unsigned int new_start = std::min(curr_start, curr_end);
    const unsigned int new_end = std::max(curr_start, curr_end);

    // 处理区间合并
    if (!results->empty()) {
      unsigned int merge_start = new_start;
      unsigned int merge_end = new_end;
      ...
      for (auto it = results->begin(); it != results->end(); ++it) {
        ...
        // exist.end
        // - 相邻条件：merge_end + 1 == exist.start 或 exist.end + 1
        // == merge_start
        bool is_overlap =
